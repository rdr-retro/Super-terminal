...
// ---------------------------------------------------------------
// Función para detectar URL en un texto desde posición 'start', devuelve true y longitud si hay URL
// ---------------------------------------------------------------
bool esURL(const std::string& texto, size_t start, size_t& length) {
    static const std::vector<std::string> prefijos = {"http://", "https://", "www."};
    static const std::vector<std::string> dominios = {".com", ".es", ".io", ".net", ".org", ".gov", ".edu"};

    for (const auto& pref : prefijos) {
        if (texto.compare(start, pref.size(), pref) == 0) {
            size_t endPos = texto.find_first_of(" \t\n\r", start + pref.size());
            if (endPos == std::string::npos) endPos = texto.size();
            length = endPos - start;
            return true;
        }
    }
    for (const auto& dom : dominios) {
        size_t posDom = texto.find(dom, start);
        if (posDom != std::string::npos && posDom >= start) {
            size_t begin = texto.rfind(' ', posDom);
...
// ---------------------------------------------------------------
// Nueva función para dibujar texto con resaltado de URLs en azul cielo
// ---------------------------------------------------------------
void dibujarTextoConURLs(sf::RenderWindow& window, const sf::Font& font, unsigned int fontSize,
                        const std::string& texto, float posX, float posY, const sf::Color& colorNormal) {
    size_t i = 0;
    float x = posX;
    sf::Text textoRender;
    ...
    while (i < texto.size()) {
        size_t urlLen = 0;
        if (esURL(texto, i, urlLen)) {
            if (i > 0) {
                std::string prev = texto.substr(0, i);
                textoRender.setString(prev);
                textoRender.setFillColor(colorNormal);
                textoRender.setPosition(x, posY);
                window.draw(textoRender);
                x += textoRender.getLocalBounds().width;
            }
            std::string url = texto.substr(i, urlLen);
            textoRender.setString(url);
            textoRender.setFillColor(sf::Color(0, 191, 255));
            textoRender.setPosition(x, posY);
            window.draw(textoRender);
            x += textoRender.getLocalBounds().width;

            std::string resto = texto.substr(i + urlLen);
            i = 0;
            if (!resto.empty()) {
                dibujarTextoConURLs(window, font, fontSize, resto, x, posY, colorNormal);
            }
            return;
        }
        i++;
...
// ---------------------------------------------------------------
// Estructura CampoTexto con función dibujar modificada para URL
// ---------------------------------------------------------------
struct CampoTexto {
    std::string texto;
    int cursorPos = 0;
    ...
    void dibujar(sf::RenderWindow& window, const sf::Font& font, unsigned int fontSize,
        ...
        if (!tieneSeleccion()) {
            dibujarTextoConURLs(window, font, fontSize, texto, rect.left + 5, rect.top + (rect.height - fontSize) / 2.f - 5, textoColor);
        } else {
            sf::Text textoRender;
            textoRender.setFont(font);
            textoRender.setCharacterSize(fontSize);
        ...
        if (activo && mostrarCursor) {
            ...
            posCursor.x = rect.left + 5 + posCursor.x;
            posCursor.y = rect.top + rect.height * 0.1f;
            sf::RectangleShape cursorShape(sf::Vector2f(2.f, rect.height * 0.8f));
            cursorShape.setFillColor(textoClaro);
            cursorShape.setPosition(posCursor.x, posCursor.y);
    ...
    while (window.isOpen()) {
                    ...
                    if (mouse.x > 200 && mouse.x < 600 && mouse.y > 270 && mouse.y < 330) {
                        if (!camaraDisponible) registroMensaje = "Cámara no disponible.";
                            ...
                            if (img.getSize().x == 0) registroMensaje = "Error capturando imagen.";
                                ...
                                // CAMBIO: Generar clave de 20 caracteres
                                claveGeneradaRegistro = gen.generateKeyFromImage(img, 20);
        ...
        if (estado == Estado::MenuPrincipal) {
            ...
            for (size_t i = 0; i < entradasCapsula.size(); ++i) {
                ...
                // Para servicio y usuario con detección URL y color azul cielo
                dibujarTextoConURLs(window, font, 20, entradasCapsula[i].servicio, xServicio, yFila + 7, textoClaro);
                dibujarTextoConURLs(window, font, 20, entradasCapsula[i].usuario, xUsuario, yFila + 7, textoClaro);

                sf::Text txtClave, btnMostrar, btnBorrar;
